...
async function sendWhatsAppMessageOnActiveIcabbiStatus(user_id) {
  try {
    ...
    user = JSON.parse(JSON.stringify(user));
    if (user) {
      const data = await sendDoubletickWhatsAppMessage(user.country_code + user.mobile_no, user.first_name, "", user.user_id, 'icabbi_driver_ref_app_id_update_v3');
      return data;
    }
  } catch (error) {
    return error.message
...
async function sendDoubletickWhatsAppMessage(mobileNo, driverName, pendingDocuments, user_id, templateName) {
  try {
    const templateMap = new Map([
      ...
      ['icabbi_driver_ref_app_id_update_v3', 8]
    ]);
